...
本ドキュメントは、`CargoOpmWorkpro` プロジェクトをテンプレートとして、新しい資格試験アプリを立ち上げるための標準的な手順を定義します。
...
2.  **テンプレートファイルのコピー**:
    - `cargo-opm-workpro-app` プロジェクトから、以下のファイルとフォルダをすべて新しいプロジェクトフォルダへコピーします。
      - `app/` （`build/` フォルダは除く）
      - `gradle/`
      - `.github/`
      - `master_data/`
...
1.  **科目（カテゴリ）定義**:
    ...
    - `app/src/main/java/com/msaitodev/cargo/opm/workpro/notifications/QuizCategoryNameProvider.kt` の `when` 式を、新しい科目定義に合わせて修正。
